...
| `fixed_assay_curves.py` | Recomputes support-size curves on a *fixed* assay set vs the naive varying-population curve |
...
| `plot_fsmol_gnn_diagram.py` | Architecture diagram for the FSMolGNN encoder (thesis figure) |
...
**Line plot** averages per-assay ΔAUPRC across seeds, then plots mean ± std error bars.  
**Boxplot** averages per-assay ΔAUPRC across seeds first, then shows the distribution across assays as boxes (one subplot per eval split).
...
python Analysis/model/fixed_assay_curves.py  # CONFIG: point CSV at a baseline_grid.csv
```
